...
- **主采集源（配置后）：** [SocialCrawl](https://www.socialcrawl.dev) App Store 评论接口
  `GET /v1/app_store/app-reviews`，固定参数 `app_id`、`country=US`、`language=en`、
  `depth=500`、`sort_by=most_recent`。认证使用服务端持有的 `x-api-key`
  （来自 `SOCIALCRAWL_API_KEY`）。每次「检查/重新检查」都会生成一个新的
  `Idempotency-Key`（同一次请求的重试复用该值），并发送 `Cache-Control: no-cache`
  以绕过 SocialCrawl 的共享缓存；`cached` 信号与采集时间戳仍然可见 —— 这不承诺 Apple
  写入评论后的零延迟可见性。当前文档计费为每次成功的 App Store 评论调用 5 credits
  （调整预算前请核实服务商定价）。应用层样本上限固定为 500 条 —— SocialCrawl 支持最多
  600，但本项目不因服务商上限而扩大分析样本。配置方式见「模型提供方与配置」。
...
- **部分失败**（某页失败但之前的页已成功）会继续用已采集的评论进行分析，并把限制信息
  传播到所有地方。
- **本地评论缓存与混合来源。** 实时评论（无论来自 SocialCrawl 还是 RSS 降级）都会合并进
...
**数据采集平台（SocialCrawl）：** 在 **设置 → 数据采集平台** 的密码输入框中保存
SocialCrawl API Key，或直接在本机 `.env.local` 中设置 `SOCIALCRAWL_API_KEY=`。保存/清除
立即生效，无需重启。该 Key 仅由服务端持有并用于实时采集 App Store 评论，绝不会进入浏览器
bundle、HTTP 响应、日志、预览 JSON、运行快照或 git 跟踪文件 —— 面板只显示「已配置 / 未配置」，
不提供查看、复制或掩码尾号。请勿把真实 Key 粘贴进 README、`.env.example`、截图或 issue 文本；
在本对话中披露过的 Key 应先在 SocialCrawl 控制台轮换再使用。测试仅允许回环地址覆盖
`SOCIALCRAWL_BASE_URL`（生产必须使用 `https://www.socialcrawl.dev`）。
