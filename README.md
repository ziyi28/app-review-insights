...
> [!IMPORTANT]
> **模型选型建议与实测对比说明：**
> - **强烈推荐 DeepSeek 系列模型（实测效果最佳）**：在全链路真实运行中，强烈建议优先使用 **`DeepSeek` 系列模型**（如 `deepseek-v4-pro`、`deepseek-v4-flash` 等）。该系列模型对复杂结构化 JSON 规范、严格字段契约（Schema Constraints）以及评论原文精确引用的遵从度最高，**输出极其稳定，流水线中间的自愈修订次数显著更少，分析质量最佳**。
> - **思考强度与分析耗时权衡**：越强的思考强度（Reasoning Effort 如 `high` / `max`）或使用旗舰推理模型（如 `deepseek-v4-pro`），分析耗时会相对较长，但产出的痛点洞察、争议剖析与 PRD 需求规划会**更加全面且深刻**；若追求极速体验，可选用轻量模型（如 `gpt5.6luna`）或适当调整推理强度（单次分析约 2~5 分钟）。
> - **更换其他模型（如 Qwen 系列等）的注意事项**：当前流水线各阶段对模型输出实施严格确定性校验。虽然系统底层已增加对各家模型输出风格的容错规范化处理，但若遇到模型在长上下文分块分析中偶发偏差，仍建议优先切回 DeepSeek 系列以获得最佳体验。
